...
import { getTabi3eenSection } from "@/lib/actions/tabi3eenActions";
...
export default function CourseViewer({
  ...
  const fetchSectionContent = async (id: string) => {
    ...
    try {
      ...
      switch (true) {
        case pathname.includes("Meditate-Quran"):
          data = await getMeditateSection(id);
          break;

        case pathname.includes("sahaba-1"):
          data = await getSahaba1Section(id);
          break;

        case pathname.includes("sahaba-2"):
          data = await getSahaba2Section(id);
          break;

        case pathname.includes("tabi3een"):
          data = await getTabi3eenSection(id);
          break;
            ...
            <nav className="space-y-1.5">
              {filteredContent.map((section, index) => (
                <button
                  key={section.id}
                  onClick={() =>
                    scrollToSection(section.id, !!section.pageLink)
                  }
                        ...
                        {hasPageLink ? (
                          <Link
                            ...
                            ${
                              darkMode
                                ? "bg-amber-600 hover:bg-amber-500 text-white"
                                : "bg-amber-500 hover:bg-amber-600 text-white shadow-sm hover:-translate-y-0.5"
                            }`}
                          >
                            <ClipboardEdit size={18} />
                            إختبار
                          ...
                          <button
                            onClick={
                              isPodcast
                                ? (e) => {
                                    e.stopPropagation();
                                    handleToggleCard(section.id, hasPageLink);
                                  }
                                : undefined
                            }
